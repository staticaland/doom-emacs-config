...
heading. The default behaviour is to look at the length of the =outline-regexp=
match. You can override the logic by setting =outline-level= to a function that
returns a integer based on your calculation of choice.
