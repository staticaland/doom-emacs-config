...
#+hugo_base_dir: ~/Projects/org-hugo-book
#+hugo_section: docs
...
(when (string= (system-name) "Anderss-MacBook-Pro.local")
  (setq doom-font (font-spec :family "Menlo" :height 160)
...
[[https://www.python.org/dev/peps/pep-0008/#blank-lines][blank lines]]).

I agree that this makes the code easier to navigate (with the eyes). Therefore
it makes little sense for =outline-minor-mode= to remove this spacing. I hope to
find some way to force two blank lines between outline headings.
